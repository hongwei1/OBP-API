...
class Nt1c3MfTest extends FunSuite with Matchers{
  ...
  test("getNt1c3Mf gets result containing proper keywords") {
    val result =  getNt1c3Mf("./src/test/resources/nt1c_3_result.json")
    assert(result.contains("TA1TSHUVATAVLAIT1"))
  }
  
  test("getFutureTransaction extract case class object") {
    val result = getFutureTransactions("./src/test/resources/nt1c_3_result.json")
    result.TA1TSHUVATAVLAIT1.TA1_SHETACH_LE_SEND_NOSAF.TA1_COUNTER should be ("4")
    result.TA1TSHUVATAVLAIT1.TA1_SHETACH_LE_SEND_NOSAF.TA1_TNUOT.TA1_PIRTEY_TNUA(2).TA1_TNUA_BODEDET.TA1_TA_ERECH should be ("20170523")
  }
