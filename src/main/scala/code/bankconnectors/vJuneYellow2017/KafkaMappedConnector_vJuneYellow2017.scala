...
import code.transactionrequests.TransactionRequests.{TransactionChallengeTypes, TransactionRequest, TransactionRequestBodyAllTypes, TransactionRequestChallenge, TransactionRequestCharge, TransactionRequestStatus, TransactionRequestTransferToAccount, TransactionRequestTransferToAtm, TransactionRequestTransferToPhone, TransactionRequestTypes}
...
object KafkaMappedConnector_vJuneYellow2017 extends KafkaMappedConnector_vJune2017 with KafkaHelper with MdcLoggable {
  ...
  override def createTransactionAfterChallengev300(
    ...
    for {
      ...
      details: TransactionRequestBodyAllTypes = tr.body
      ...
      transactionRequestCommonBody <-TransactionRequestTypes.withName(transactionRequestType.value) match {
        case TRANSFER_TO_PHONE => tryo {(details.to_transfer_to_phone).get} ?~! s"$TransactionRequestDetailsExtractException It can not extract to $TransactionRequestTransferToPhone "
        case TRANSFER_TO_ATM =>tryo {(details.to_transfer_to_atm).get} ?~! s"$TransactionRequestDetailsExtractException It can not extract to $TransactionRequestTransferToAtm "
        case TRANSFER_TO_ACCOUNT =>tryo {(details.to_transfer_to_account).get} ?~! s"$TransactionRequestDetailsExtractException It can not extract to $TransactionRequestTransferToAccount "
        case _ =>tryo {(details.to_transfer_to_phone).get} ?~! s"$TransactionRequestDetailsExtractException It can not extract to $TransactionRequestTransferToPhone "
      }
