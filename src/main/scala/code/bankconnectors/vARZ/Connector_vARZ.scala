...
trait Connector_vARZ extends Connector with KafkaHelper with MdcLoggable {
  ...
  messageDocs += MessageDoc(
    ...
    outboundAvroSchema = Some(parse(SchemaFor[OutboundCreateChallengeJune]().toString(true))),
  ...
  override def createChallenge(bankId: BankId, accountId: AccountId, userId: String, transactionRequestType: TransactionRequestType, transactionRequestId: String, callContext: Option[CallContext] = None) = {
    ...
    val box = for {
      authInfo <- getAuthInfo(callContext)
      req = OutboundCreateChallengeJune(
        authInfo = authInfo, 
        bankId = bankId.value,
        accountId = accountId.value,
        userId = userId,
      ...
      kafkaMessage <- processToBox[OutboundCreateChallengeJune](req)
