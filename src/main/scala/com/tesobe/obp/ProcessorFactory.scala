  ...
  def getProcessor = {
    processorName match {
      case "localFile" => Seq(
        ...
        BusinessTopic(createTopicByClassName(OutboundGetCustomersByUserIdFuture.getClass.getSimpleName), LocalProcessor()(executionContext, materializer).getCustomerFn)
