...
trait MappedDecoder {
        ...
        extractQuery(request) match {
          case Some("obp.get.Bank") =>
            example.banks.filter(_.id == Some(request.bankId)).headOption match {
              ...
              case None => Map("data" -> InboundBank(List(BackendMessage("ESB","Success", "0", "OK")),BankNotFound, "", "", "", "")).asJson.noSpaces
            }
  ...
  def mapBankToInboundBank(x: Bank) = {
    InboundBank(List(BackendMessage("ESB","Success", "0", "OK")),"", x.id.getOrElse(""), x.fullName.getOrElse(""), x.logo.getOrElse(""), x.website.getOrElse(""))
  }
