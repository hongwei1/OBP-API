...
object CustomDB extends DB1 {
  private[db] lazy val theDB = new CustomDB{}
}

trait CustomDB extends Loggable {
  ...
  private def newConnection(name: ConnectionIdentifier): SuperConnection = {
    ...
    new SuperConnection(conn,()=>conn.close())//no need close the connection if we use library!
  }
      ...
      def apply[T](f: => T): T = if (DepthCnt.is == Full(true)) f
        ...
        if (eager) {
          def recurseMe(lst: List[ConnectionIdentifier]): T = lst match {
            ...
            case x :: xs => CustomDB.use(x) {ignore => recurseMe(xs)}
          }
          recurseMe(in)
        } else {
   ...
   * the context of a buildLoanWrapper or a CustomDB.use {}
   */
   ...
   * CustomDB.userReservedWords = Full(Set("foo", "bar"))
   */
