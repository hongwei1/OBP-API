...
object UserAttributeProvider extends SimpleInjector {
  ...
  def buildOne: UserAttributeProvider = DoobieUserAttributeProvider
...
trait UserAttributeProvider extends MdcLoggable {
  ...
  def getUserAttributesByUser(userId: String): Future[Box[List[UserAttributeTrait]]]
  def getPersonalUserAttributes(userId: String): Future[Box[List[UserAttributeTrait]]]
  def getNonPersonalUserAttributes(userId: String): Future[Box[List[UserAttributeTrait]]]
  def getUserAttributesByUsers(userIds: List[String]): Future[Box[List[UserAttributeTrait]]]
