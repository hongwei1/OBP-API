...
object DoobieConsumersProvider extends ConsumersProvider with MdcLoggable {
  ...
  private def toEntity(row: ConsumerRow): Consumer = {
    ...
    // MappedLongIndex.writePermission_? = false, so c.id(...) throws outside a safe block.
    // runSafe sets safe_? = true for this object, mirroring how Lift Mapper initialises
    // fields when loading from the database.
    c.runSafe {
      c.id(row.id)
