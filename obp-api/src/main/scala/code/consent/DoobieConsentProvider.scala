...
object DoobieConsentProvider extends ConsentProvider with MdcLoggable {
  ...
  override def saveUKConsent(
    ...
    // 0..1 per the UK spec: absent means open-ended, persisted as NULL rather than an epoch value.
    // Kept as Option — Doobie binds Option[Timestamp] as a proper SQL NULL, whereas a raw null
    // Timestamp fails to bind ("Expected non-nullable...") and aborts the insert.
    val expirationTs    = expirationDateTime.map(d => new Timestamp(d.getTime))
    val fromTs          = transactionFromDateTime.map(d => new Timestamp(d.getTime))
    val toTs            = transactionToDateTime.map(d => new Timestamp(d.getTime))
    // Bind nullable text columns as Option[String] — Doobie's Put[String] rejects a raw null.
    val userId: Option[String] = user.map(_.userId)
