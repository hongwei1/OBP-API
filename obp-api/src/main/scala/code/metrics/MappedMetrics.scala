        ...
        DoobieUtil.runQuery(
          sql"""UPDATE metricarchive
                   SET metricid = $primaryKey, userid = $userId, url = $url, date_c = $ts,
                       ...
                       certificate_trust = $certificateTrust,
                       certificate_trust_detail = $certificateTrustDetail
          ...
          DoobieUtil.runQuery(
            sql"""INSERT INTO metricarchive
                    (metricid, userid, url, date_c, duration, username, appname, developeremail,
                     ...
                     consent_reference_id, httpcode, certificate_trust, certificate_trust_detail)
                  VALUES
                    ($primaryKey, $userId, $url, $ts, $duration, $userName, $appName, $developerEmail,
                     $consumerId, $implementedByPartialFunction, $implementedInVersion, $verb,
                     $correlationId, $responseBody, $sourceIp, $targetIp, $apiInstanceId,
                     $consentReferenceId, $httpCodeVal, $certificateTrust, $certificateTrustDetail)""".update.run
          ) > 0
        } catch {
    ...
    CacheKeyFromArguments.buildCacheKey {
      Caching.memoizeSyncWithProvider(Some(cacheKey.toString()))(cacheTTL.seconds) {
        val conditions: List[Fragment] = List(
          ...
          // certificate_trust is a live query param (APIUtil parses it into OBPCertificateTrust).
          // The Lift filter builder this replaced applied it; without this fragment the parameter
          // would be accepted and then silently ignored, returning unfiltered results.
          queryParams.collectFirst { case OBPCertificateTrust(v)               => fr"certificate_trust = $v" },
