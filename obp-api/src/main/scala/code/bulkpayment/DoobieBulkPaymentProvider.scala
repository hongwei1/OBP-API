...
object DoobieBulkPaymentProvider extends BulkPaymentProvider {
  ...
  // Mirrors the Mapped implementation: delete the claim row matching all four keys. A claim that
  // is not there is not an error (Lift's find(...).foreach(_.delete_!) is a no-op when Empty), so
  // the row count is deliberately ignored.
  override def releaseBatchReference(fromBankId: String, fromAccountId: String, batchReference: String,
    ...
    DoobieUtil.runQuery(
      sql"""DELETE FROM bulkbatchreference
            WHERE frombankid = ${nn(fromBankId)}
              AND fromaccountid = ${nn(fromAccountId)}
              AND batchreference = ${nn(batchReference)}
              AND transactionrequestid = ${nn(transactionRequestId)}""".update.run)
    ()
  }
}
