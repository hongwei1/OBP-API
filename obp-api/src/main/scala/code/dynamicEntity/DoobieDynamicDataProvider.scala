...
object DoobieDynamicDataProvider extends DynamicDataProvider {
  ...
  // Community-scoped update: the row is located without the owner/personal filter, but its existing
  // owner and personal flag are preserved — a row-level access change edits the data, not its
  // provenance. This mirrors MapppedDynamicDataProvider.updateCommunity.
  override def updateCommunity(bankId: Option[String], entityName: String, requestBody: JObject, id: String): Box[DynamicDataT] =
    getCommunity(bankId, entityName, id).flatMap { existing =>
      update(bankId, entityName, requestBody, id, existing.userId, existing.isPersonalEntity)
    }

  // Community-scoped delete, mirroring MapppedDynamicDataProvider.deleteCommunity: locate without
  // the owner filter, delete, then drop the projection row (a no-op unless projection is enabled).
  override def deleteCommunity(bankId: Option[String], entityName: String, id: String): Box[Boolean] =
    getCommunity(bankId, entityName, id).map { _ =>
      val rows = DoobieUtil.runQuery(
        sql"DELETE FROM dynamicdata WHERE dynamicdataid = $id AND dynamicentityname = ${nn(entityName)}".update.run)
      code.api.dynamic.entity.projection.ProjectionDualWrite.onDelete(bankId, entityName, id)
      rows >= 0
