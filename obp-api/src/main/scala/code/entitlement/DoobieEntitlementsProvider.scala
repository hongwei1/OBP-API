...
object DoobieEntitlementsProvider extends EntitlementProvider {
  ...
  private case class DoobieEntitlement(row: EntRow) extends Entitlement {
    ...
    // Audit metadata: who granted this entitlement. Empty is treated as absent, matching the
    // Lift implementation in MappedEntitlements.
    override def grantedByUserId: Option[String] = {
      val g = row.granted_by_user_id.orNull
  ...
  private val selectCols: Fragment =
    fr"""SELECT mentitlementid, mbankid, muserid, mrolename, mcreatedbyprocess,
                group_id, process, entitlement_request_id, granted_by_user_id
  ...
  override def addEntitlement(bankId: String,
                              ...
                              // Audit only, stored as-is. Authorization belongs to the calling
                              // endpoint: the grantor role-gate this provider used to apply was
                              // removed upstream (2026-08-09) because no caller passed a grantor
                              // and the check ignored super admins, whose granting rights are
                              // virtual and have no rows to find.
                              grantedByUserId: Option[String] = None,
                              groupId: Option[String] = None,
                              process: Option[String] = None): Box[Entitlement] = {
    // tryo, matching the Mapped implementation's tryo(entitlement.saveMe()): mappedentitlement
    // carries UniqueIndex(mbankid, muserid, mrolename), so granting the same role twice is a
    // constraint violation. Lift turned that into a Failure; without tryo it escapes as an
    // exception. (Invisible until the fallback transactor started committing — before that the
    // duplicate insert was rolled back and never reached the index.)
    def addEntitlementToUser(): Box[Entitlement] = tryo {
      val newId = APIUtil.generateUUID()
      val now = new java.sql.Timestamp(System.currentTimeMillis())
      val groupIdStr = nn(groupId.getOrElse(""))
      val processStr = nn(process.getOrElse(""))
      val grantedByStr = nn(grantedByUserId.getOrElse(""))
      DoobieUtil.runQuery(
        sql"""INSERT INTO mappedentitlement
                (mentitlementid, mbankid, muserid, mrolename, mcreatedbyprocess, group_id, process, granted_by_user_id, createdat, updatedat)
              VALUES ($newId, ${nn(bankId)}, ${nn(userId)}, ${nn(roleName)}, ${nn(createdByProcess)},
                      $groupIdStr, $processStr, $grantedByStr, $now, $now)""".update.run)
      val addEntitlement = DoobieEntitlement(EntRow(
        ...
        Some(groupIdStr), Some(processStr), None, Some(grantedByStr)))
