    ...
    ConsentsJsonV510(
      ...
      consents = consents.map { c =>
        ...
        AllConsentJsonV510(
          ...
          frequency_per_day = if(c.apiStandard == BerlinGroupVocabulary.berlinGroupVersion1.apiStandard) Some(c.frequencyPerDay) else None,
          remaining_requests = if(c.apiStandard == BerlinGroupVocabulary.berlinGroupVersion1.apiStandard) Some(c.frequencyPerDay - c.usesSoFarTodayCounter) else None,
          api_standard = c.apiStandard,
          api_version = c.apiVersion,
          note = c.note
        )
