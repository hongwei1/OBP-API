...
object MigrationOfCustomerAttributes {
  ...
  def alterColumnValue(name: String): Boolean = {
    DbFunction.tableExists(MappedCustomerAttribute, (DB.getConnection(DefaultConnectionIdentifier))) match {
      case true =>
  ...
  def populateAzpAndSub(name: String): Boolean = {
    DbFunction.tableExists(Consumer, (DB.getConnection(DefaultConnectionIdentifier))) match {
      case true =>
        val startDate = System.currentTimeMillis()
        val commitId: String = APIUtil.gitCommit
        var isSuccessful = false
