...
object DoobieViewProvider {
  ...
  private def toViewDefinition(row: ViewRow): ViewDefinition = {
    ...
    // canGrantAccessToCustomViews / canRevokeAccessToCustomViews are no longer columns on
    // ViewDefinition: they are derived from the ViewPermission table via hasPermission(...).
    // Setting them here is not possible and would not be read — permission resolution happens
    // through allowed_actions, as it does for every other view permission.
