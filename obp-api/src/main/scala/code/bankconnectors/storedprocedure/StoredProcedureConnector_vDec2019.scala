...
// ---------- created on 2020-08-17T23:13:49Z
  ...
  def getBankAccountsBalancesDoc = MessageDoc(
    ...
    exampleInboundMessage = (
     InBoundGetBankAccountsBalances(inboundAdapterCallContext=MessageDocsSwaggerDefinitions.inboundAdapterCallContext,
      ...
      amount=amountExample.value),
      overallBalanceDate=toDate(dateExample)))
    ),
    adapterImplementation = Some(AdapterImplementation("- Core", 1))
  )
  ...
  def createAccountApplicationDoc = MessageDoc(
    ...
    exampleInboundMessage = (
     InBoundCreateAccountApplication(inboundAdapterCallContext=MessageDocsSwaggerDefinitions.inboundAdapterCallContext,
      ...
      userId=userIdExample.value,
      customerId=customerIdExample.value,
      dateOfApplication=toDate(dateExample),
      status="string"))
    ),
    adapterImplementation = Some(AdapterImplementation("- Core", 1))
  )
  ...
  override def checkAnswer(paymentId: String, authorizationId: String, challengeData: String, callContext: Option[CallContext]): OBPReturnType[Box[AuthorisationTrait]] = {
        import com.openbankproject.commons.dto.{InBoundCheckAnswer => InBound, OutBoundCheckAnswer => OutBound}  
        val req = OutBound(callContext.map(_.toOutboundAdapterCallContext).orNull, paymentId, authorizationId, challengeData)
