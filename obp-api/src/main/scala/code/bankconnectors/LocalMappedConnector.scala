...
object LocalMappedConnector extends Connector with MdcLoggable {
  ...
  override def getBankAccountByRoutingLegacy(bankId: Option[BankId], scheme: String, address: String, callContext: Option[CallContext]): Box[(BankAccount, Option[CallContext])] = {
    ...
    // table without touching BankAccountRouting. See OBP-30545.
  ...
  override def getAccountRouting(bankId: Option[BankId], scheme: String, address: String, callContext: Option[CallContext]): Box[(BankAccountRouting, Option[CallContext])] = {
    ...
    // (write paths reject them with OBP-30545). This lookup is used as a
    // uniqueness check on routing-row creation, so for OBP-family it must
    // always report "no existing row" — otherwise virtual + stored could be
    // ambiguous. Account lookups by OBP scheme go through
    // getBankAccountByRouting, not here.
    if (isImplicitOBPAccountScheme(scheme)) {
      Empty
    } else {
      bankId match {
  ...
  override def createTransactionAfterChallengeV210(fromAccount: BankAccount, transactionRequest: TransactionRequest, callContext: Option[CallContext]): OBPReturnType[Box[TransactionRequest]] = {
    for {
      ...
      (transactionId, callContext) <- TransactionRequestTypes.withName(transactionRequestType) match {
        ...
        // OPEN_CORRIDOR: same inline-routing payment shape as SIMPLE, but with a
        // mandatory `originator` block carried on the create body (validated by
        // JSON deserialization into TransactionRequestBodyOpenCorridorJsonV700,
        // and persisted onto MappedTransactionRequest by
        // createTransactionRequestImpl210). The originator is for FATF Rec 16
        // compliance — it travels with the payment. The downstream Open Corridor
        // workflow (Cardano Promise, netting, settlement) hooks in around this
        // payment step in subsequent commits; for now the money movement itself
