...
class DoobieAtmsProviderTest extends ServerSetup {
  ...
  class DefaultSetup {
      ...
      val unlicensedAtm: AtmT = DoobieAtmsProvider.createOrUpdateAtm(
      ...
      val atm1: AtmT = DoobieAtmsProvider.createOrUpdateAtm(
      ...
      val atm2: AtmT = DoobieAtmsProvider.createOrUpdateAtm(
  ...
  feature("DoobieAtmsProvider") {
    ...
    scenario("We try to get atms") {
      ...
      DoobieAtmsProvider.getAtms(BankId(fixture.bankIdX), List(OBPLimit(1000))).get.nonEmpty should equal(true)

      When("we try to get the atms for that bank")
      val atmsOpt: Option[List[AtmT]] = DoobieAtmsProvider.getAtms(BankId(fixture.bankIdX),List(OBPLimit(1000))) //OBPLimit(1000) is just a place holder
    ...
    scenario("We try to get atms for a bank that doesn't have any") {
      ...
      DoobieAtmsProvider.getAtms(BankId(fixture.bankIdY), List(OBPLimit(1000))).get.isEmpty should equal(true)

      When("we try to get the atms for that bank")
      val atmDataOpt = DoobieAtmsProvider.getAtms(BankId(fixture.bankIdY), List(OBPLimit(1000))) //OBPLimit(1000) is just a place holder
