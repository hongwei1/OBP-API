#!/bin/bash
################################################################################
# OBP-API JDK selection (sourced by the build/run scripts)
#
# The build and the shipped artifacts are pinned to one JDK: <java.version> in
# pom.xml, which is also what every CI job and every Dockerfile uses. Running a
# different JDK is not a supported configuration - it changes both what compiles
# and what the server runs on - so this file selects that JDK up front and
# refuses to continue if it is missing.
#
...
#      has been misread that way before. (pom.xml now also enforces this via
#      maven-enforcer-plugin; this check just fails earlier and more clearly.)
...
java_env_required="$(sed -nE 's/.*<java\.version>([0-9]+)<\/java\.version>.*/\1/p' "$java_env_pom" 2>/dev/null | head -1)"
...
# java_env_is_required <java-home-or-binary>: true iff it reports exactly $java_env_required.
